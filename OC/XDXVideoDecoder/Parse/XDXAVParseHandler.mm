...
- (int)getAVStreamIndexWithFormatContext:(AVFormatContext *)formatContext isVideoStream:(BOOL)isVideoStream {
    ...
    if (avStreamIndex == -1) {
        log4cplus_error(kModuleName, "%s: Not find video stream",__func__);//__func__宏名称表示当前函数的名称
        return NULL;
    }else {
...
- (void)startParseGetAVPacketWithFormatContext:(AVFormatContext *)formatContext videoStreamIndex:(int)videoStreamIndex audioStreamIndex:(int)audioStreamIndex completionHandler:(void (^)(BOOL isVideoFrame, BOOL isFinish, AVPacket packet))handler{
    ...
    dispatch_async(parseQueue, ^{//异步处理
        ...
        while (!self->m_isStopParse) {
            ...
            if ((size < 0) || (packet.size < 0)) {//这里会退出流数据的读取
    ...
    dispatch_async(parseQueue, ^{
        ...
        while (!self->m_isStopParse) {
            ...
            if (packet.stream_index == videoStreamIndex) {
                ...
                //                int iphoneType = [self iphoneTypeInt];
                //                log4cplus_info(kModuleName, "iphoneType is %d, %s", iphoneType, __func__);
                //                if ((videoInfo.videoRotate != 0) && (iphoneType <= 8)) {//这里暂不支持有角度的视频
                ...
                //过滤一下，每个视频帧前添加startcode,I帧前还会添加SPS,PPS
...
//                log4cplus_info(kModuleName, "%s: packet size: %d, new packet size: %d",__func__, packet.size, new_packet.size);
                ...
                /*
                 这里的代码存在很大的疑问，后面再分析吧，没时间了
                 videoInfo.data          = video_data;
                 videoInfo.dataSize      = video_size;
                 为什么不是：
                 videoInfo.data          = new_packet.data;
                 videoInfo.dataSize      = new_packet.size;
                 */
                
//                NSLog(@"-------------video data------------");
//                NSLog(@"%p",video_data);
//                NSMutableString *videoDataString = [NSMutableString string]; 
//                for (int i = 0; i < 1000; i++) {
//                    [videoDataString appendFormat:@"0x%02x ", video_data[i]]; 
//                    if ((i + 1) % 16 == 0) {
//                        [videoDataString appendString:@"\n"]; 
//                    }
//                } 
//                NSLog(@"%@", videoDataString);
//
//                NSLog(@"-------------new packet data------------");
//                NSLog(@"%p",new_packet.data);
//                NSMutableString *newPacketDataString = [NSMutableString string]; 
//                for (int i = 0; i < 1000; i++) {
//                    [newPacketDataString appendFormat:@"0x%02x ", new_packet.data[i]]; 
//                    if ((i + 1) % 16 == 0) {
//                        [newPacketDataString appendString:@"\n"]; 
//                    }
//                } 
//                NSLog(@"%@", newPacketDataString);
...
//                    [extraDataString appendFormat:@"0x%02x ", videoInfo.extraData[i]]; 
//                    if ((i + 1) % 16 == 0) {
//                        [extraDataString appendString:@"\n"]; 
//                    }
//                } 
//                NSLog(@"%@", extraDataString);
            ...
            if (packet.stream_index == audioStreamIndex) {
                ...
                audioInfo.dataSize = packet.size;//音频包大小
                audioInfo.channel = formatContext->streams[audioStreamIndex]->codecpar->channels;//音频通道
                audioInfo.sampleRate = formatContext->streams[audioStreamIndex]->codecpar->sample_rate;//音频采样率
                audioInfo.pts = packet.pts * av_q2d(formatContext->streams[audioStreamIndex]->time_base);//音频时间戳
                // send audio info
                if (handler) {
